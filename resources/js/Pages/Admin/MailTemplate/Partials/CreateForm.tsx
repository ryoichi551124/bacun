...
import createMailTemplateSchema, {
  CreateMailTemplateSchemaType,
} from '@/Schemas/Admin/MailTemplate/createSchema'
...
export default function MailTemplateCreateForm() {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<CreateMailTemplateSchemaType>({
    reValidateMode: 'onBlur',
    resolver: zodResolver(createMailTemplateSchema),
  })

  /** メールテンプレート新規作成　*/
  const createMailTemplate = (data: CreateMailTemplateSchemaType) => {
    router.post('/admin/mailtemplate/create/create', data)
  }

  return (
    <Card title="メールテンプレート新規作成">
      <form onSubmit={handleSubmit(createMailTemplate)} css={forms.container}>
