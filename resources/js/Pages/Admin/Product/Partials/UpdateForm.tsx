...
import { productToFormData } from '@/Services/products/productSevice'
...
export default function ProcutUpdateForm({
  ...
  const { product, categories, deliveries, types, tags, statuses } =
    usePage<ProductData>().props
  const [isUnLimited, setIsUnLimited] = useState<boolean>(product.stock > 99)
  ...
  const onSubmit = (data: UpdateProductSchemaType) => {
    ...
    onUpdateProduct && onUpdateProduct(data)
  }
